...
export function formatGithubComment(annotatedLinks) {
  ...
  for (const annotatedLink of annotatedLinks.sort((a, b) => a.url.localeCompare(b.url))) {
    ...
    comment += `${multiple ? "\n* " : "" } ${state} in [${annotatedLink.title} ${annotatedLink.context ? ` (${annotatedLink.context.title})`: ""}](${annotatedLink.url}#${annotatedLink?.context?.id || ''})`;
...
export async function updateGithub(octokit, annotatedLinks, {dryRun, ignoreRepos = []}) {
  ...
  for (const link of Object.keys(groupedLinks)) {
    ...
    if (ignoreRepos.includes(`${owner}/${repo}`)) {
      console.log(`Skipping commenting on ${link} per configuration`);
      continue;
    }
